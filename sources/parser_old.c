...
/*   Updated: 2024/05/16 14:37:54 by juitz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**store_input_in_struct(t_minishell *ms)
{
	...
	while (current)
	{
		...
		{
			ms->cmd->full_cmd[i] = ft_strdup(current->attr);
			if (ms->cmd->full_cmd[i] == NULL)
...
char **split_command(t_minishell *ms)
{
	...
	i = 0;
	j = 0;
	while (ms->cmd->full_cmd[i])
	{
		if (ms->cmd->full_cmd[i][0] == '|')
		{
			i++;
			j++;
		}
		ms->cmd->simp_cmd[j] = ft_strjoin(ms->cmd->simp_cmd[j], ms->cmd->full_cmd[i]);
		if (ms->cmd->simp_cmd[j] == NULL)
			return (ft_free(ms->cmd->simp_cmd), NULL);
		i++;
	}
	ms->cmd->simp_cmd[j + 1] = NULL;
