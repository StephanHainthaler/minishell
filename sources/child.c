...
/*   Updated: 2024/08/13 11:32:13 by juitz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (cmd->simp_cmd == NULL || cmd->simp_cmd[0] == NULL)
		exit_child(exec, -1, -1, 0);
	if (handle_builtins_1(exec, cmd->simp_cmd) != 1)
		exit_child(exec, -1, -1, exec->exit_status);
	if (handle_builtins_2(exec, cmd->simp_cmd) != 1)
		exit_child(exec, -1, -1, exec->exit_status);
...
void	single_child_proc(t_executor *exec, t_cmd *cmd)
{
	...
	{
		cmd->cmd_path = get_cmd_path(exec, exec->cmds[0], &error_flag, 0);
		if (error_flag == 1)
			exit_child(exec, -1, -1, -1);
	}
...
void	multi_child_proc(t_executor *exec, t_cmd *cmd, int ends[], int *old_end)
{
	...
	if (cmd->simp_cmd != NULL)
	{
		cmd->cmd_path = get_cmd_path(exec, cmd, &error_flag, 0);
		if (error_flag == 1)
			exit_child(exec, -1, -1, -1);
	}
